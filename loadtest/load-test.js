...
export function setup() {
    ...
    // 3. Profil oluştur (Users Microservice)
    http.post(`${BASE_URL}/api/users`, JSON.stringify({
        username: TEST_USER.username,
        displayName: `${TEST_USER.username}_Display`,
        bio: "Load test bio"
    }), {
        headers: {
...
export default function (data) {
    ...
    group('Users API', function () {
        const getProfile = http.get(`${BASE_URL}/api/users/by-username/${data.username}`, params);
        check(getProfile, { 'get profile successful': (r) => r.status === 200 });
    });
